function getFlick() {
	...
	$.getJSON(url, function(data){
		 ...
		 var countImg = data.photos.photo.length;
		 ...
		 for (i = 0; i < data.photos.photo.length; i++ )
			 {
				...
				newThumb.title = data.photos.photo[i].title;
				newThumb.src = url;
				newThumb.setAttribute('bigID', largeUrl);
				...
				document.getElementById('thumbnails').appendChild(newLi);		
			 }
			
			//Load the image into the main section
			var imgUrl = $('#thumbnails li:first-child > img').attr('bigID');
			loadImage(imgUrl);
			$('#search').html("Find Images");
		 }
	});
...
function moveSlide(){
	...
	var thumbMiddle = carouselWidth/2 - (activeWidth-imgMargin)-prevWidth;
...
$(document).ready(function(){
	...
	$("#thumbnails").on('click', 'li', function(){		
		...
		//Load the image into the main section
		imgUrl = $(this).find('img').attr('bigID');
		loadImage(imgUrl);
